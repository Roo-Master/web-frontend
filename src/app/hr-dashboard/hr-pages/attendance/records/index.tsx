...
import {
  FileText, Search, RefreshCw, Download, ChevronLeft,
  Users, Building2, Edit, Eye, Trash2, AlertCircle,
  Calendar, Filter
} from 'lucide-react';
...
export default function AttendanceRecords() {
  ...
  const { data, loading, error, refetch } = useApi(
    () => attendanceService.getAttendanceByDate(startDate, endDate)
  );
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div className="text-center max-w-md bg-white rounded-xl border border-gray-200 p-8">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mt-4">Failed to Load Records</h3>
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
        ...
        <div className="flex flex-wrap gap-3 mt-4 md:mt-0">
          ...
          <button onClick={() => router.push('/attendance/corrections')} className="px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 transition-colors flex items-center gap-2 text-sm">
            <Edit className="w-4 h-4" /> Corrections
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-2 mb-6 flex gap-2">
        <button
          ...
          className={`flex-1 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
            view === 'employee' ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-50'
          }`}
        >
          <Users className="w-4 h-4 inline mr-2" /> Employee Records
        </button>
        <button
          onClick={() => setView('department')}
          className={`flex-1 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
            view === 'department' ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-50'
          }`}
        >
          <Building2 className="w-4 h-4 inline mr-2" /> Department Records
        </button>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="relative">
            ...
            <input 
              type="text" 
              placeholder={view === 'employee' ? 'Search employees...' : 'Search departments...'}
              value={search}
            ...
            <thead className="bg-gray-50">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  {view === 'employee' ? 'Employee' : 'Department'}
                </th>
            ...
            <tbody>
              {filteredRecords.map((record: any) => (
                <tr key={record.id} className="border-t border-gray-100 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-600">{record.date ? new Date(record.date).toLocaleDateString() : '--'}</td>
                  <td className="px-4 py-3">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                      record.status === 'present' ? 'bg-green-100 text-green-700 border border-green-300' :
                      record.status === 'absent' ? 'bg-red-100 text-red-700 border border-red-300' :
                      record.status === 'late' ? 'bg-yellow-100 text-yellow-700 border border-yellow-300' :
                      record.status === 'on_leave' ? 'bg-blue-100 text-blue-700 border border-blue-300' :
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-1">
                      ...
                      <button onClick={() => handleDeleteRecord(record.id)} className="p-1.5 hover:bg-red-50 rounded-lg transition-colors" title="Delete">
