...
import {
  Edit, RefreshCw, ChevronLeft, CheckCircle, XCircle,
  AlertCircle, Search, Filter, Eye, Clock
} from 'lucide-react';
...
export default function AttendanceCorrections() {
  ...
  const { data, loading, error, refetch } = useApi(
    () => attendanceService.getAttendanceCorrections()
  );
  ...
  const handleRefresh = async () => {
    await refetch();
    showToast('✅ Corrections refreshed!', 'success');
  };

  const handleApprove = async (id: string) => {
    if (confirm('Approve this correction request?')) {
      try {
        await attendanceService.reviewAttendanceCorrection(id, 'approved');
        await refetch();
  ...
  const handleReject = async (id: string) => {
    if (confirm('Reject this correction request?')) {
      try {
        await attendanceService.reviewAttendanceCorrection(id, 'rejected');
        await refetch();
  ...
  const filteredCorrections = corrections.filter((r: any) => {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 mt-4">Loading corrections...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div className="text-center max-w-md bg-white rounded-xl border border-gray-200 p-8">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mt-4">Failed to Load Corrections</h3>
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
        ...
        <div className="flex flex-wrap gap-3 mt-4 md:mt-0">
          ...
          <button onClick={() => router.push('/attendance/daily')} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-2 text-sm">
            <Clock className="w-4 h-4" /> Request Correction
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {filteredCorrections.map((record: any) => (
                <tr key={record.id} className="border-t border-gray-100 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-1">
                      ...
                      <button onClick={() => router.push(`/attendance/corrections/${record.id}`)} className="p-1.5 hover:bg-gray-100 rounded-lg transition-colors" title="View">
              ...
              {filteredCorrections.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-4 py-12 text-center">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">No Correction Requests</h3>
                    <p className="text-gray-500">No attendance correction requests found.</p>
                  </td>
                </tr>
              )}
            </tbody>
