...
const payrollAPI = {
  getDashboard: async () => {
    // Replace with: return await axios.get('/api/payroll/dashboard')
    return {
      status: 'Ready',
      totalMonthly: 0,
      employeesPaid: 0,
  ...
  getSalaryManagement: async () => {
    // Replace with: return await axios.get('/api/payroll/salaries')
    return {
      basicSalary: 0,
      allowances: 0,
      deductions: 0,
  ...
  getTaxSettings: async () => {
    // Replace with: return await axios.get('/api/payroll/tax-settings')
    return {
      taxRate: 0,
      taxBrackets: [],
      totalTax: 0
  ...
  calculateTax: async (amount: number) => {
    // Replace with: return await axios.post('/api/payroll/calculate-tax', { amount })
    return { tax: 0, net: amount };
  },
  getPayrollProcessing: async () => {
    // Replace with: return await axios.get('/api/payroll/processing')
    return [];
  },
  generatePayroll: async (data: any) => {
    // Replace with: return await axios.post('/api/payroll/generate', data)
    return { success: true, message: 'Payroll generated successfully' };
  },
  reviewPayroll: async (id: string) => {
    // Replace with: return await axios.patch(`/api/payroll/${id}/review`)
    return { success: true };
  },
  approvePayroll: async (id: string) => {
    // Replace with: return await axios.patch(`/api/payroll/${id}/approve`)
    return { success: true };
  },
  getPayslips: async (employeeId?: string, month?: string) => {
    // Replace with: return await axios.get('/api/payroll/payslips', { params: { employeeId, month } })
    return [];
  },
  generatePayslip: async (employeeId: string, month: string) => {
    // Replace with: return await axios.post('/api/payroll/payslips/generate', { employeeId, month })
    return { success: true, url: '/payslip.pdf' };
  },
  downloadPayslip: async (id: string) => {
    // Replace with: return await axios.get(`/api/payroll/payslips/${id}/download`)
    return { success: true };
  },
  printPayslip: async (id: string) => {
    // Replace with: return await axios.get(`/api/payroll/payslips/${id}/print`)
    return { success: true };
  },
  getPayrollHistory: async (month?: string) => {
    // Replace with: return await axios.get('/api/payroll/history', { params: { month } })
    return [];
  },
  getEmployeePayrollHistory: async (employeeId: string) => {
    // Replace with: return await axios.get(`/api/payroll/employee/${employeeId}`)
    return [];
  },
  updateSalary: async (employeeId: string, data: any) => {
    // Replace with: return await axios.put(`/api/payroll/salary/${employeeId}`, data)
    return { success: true };
  },
  exportPayroll: async (format: string, month: string) => {
    // Replace with: return await axios.get(`/api/payroll/export/${format}`, { params: { month } })
    return { success: true };
  },
  getPayrollStats: async () => {
    // Replace with: return await axios.get('/api/payroll/stats')
    return {
      draft: 0,
      reviewed: 0,
      approved: 0
...
export default function Payroll() {
  ...
  const handleReviewPayroll = async (id: string) => {
    if (confirm('Review this payroll?')) {
      try {
        ...
        showError('Failed to review payroll');
      }
    }
  };
  ...
  const handleExport = async (format: string) => {
    try {
      ...
      showError('Failed to export payroll');
    }
  };
  ...
  const handleSearchEmployeeHistory = async () => {
    ...
    try {
      ...
      if (data.length === 0) {
        showSuccess('No records found for this employee');
      }
    } catch (err) {
      showError('Failed to fetch employee records');
    }
  };
  ...
  return (
    <Layout>
            ...
            {['dashboard', 'salary', 'processing', 'payslips', 'history'].map((tab) => (
              ...
              >
                {tab === 'dashboard' ? 'Payroll Dashboard' :
                 ...
                 tab === 'payslips' ? 'Payslips' : 'Payroll History'}
              </button>
            ))}
          </nav>
        </div>
      ...
      {/* Modal for Generate Payroll / Payslip */}
      {showModal && (
            ...
            <div className="space-y-4">
              ...
              {modalType === 'payslip' && (
                <div>
                  ...
                  />
                  <label className="block text-sm font-medium text-gray-700 mt-3 mb-1">Month</label>
                  <input
                    type="month"
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
                    value={selectedMonth}
    ...
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-2">
          ...
          />
          <button onClick={onRefresh} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">
            🔄 Refresh
          </button>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-blue-500">
          ...
          <button onClick={() => alert(`Status: ${data.status || 'Ready'}`)} className="text-xs text-blue-600 hover:text-blue-800 mt-1">
            View details →
          </button>
        </div>
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-green-500">
          <p className="text-sm text-gray-500">Total Monthly Payroll</p>
          <p className="text-2xl font-bold text-green-600">${data.totalMonthly?.toLocaleString() || 0}</p>
        ...
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-yellow-500">
          ...
          <button onClick={() => alert(`${data.pendingPayments || 0} pending payments`)} className="text-xs text-yellow-600 hover:text-yellow-800 mt-1">
            View pending →
          </button>
        </div>
      </div>
...
function SalaryManagement({ data, taxData, onRefresh, onUpdate }: any) {
  ...
  const calculateNetPay = (basic: number, allowances: number, deductions: number) => {
    const tax = taxData.taxRate || 0;
    const taxAmount = (basic + allowances) * (tax / 100);
    ...
    <div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-blue-500">
          ...
          <p className="text-2xl font-bold text-gray-800">${data.basicSalary?.toLocaleString() || 0}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-green-500">
          <p className="text-sm text-gray-500">Allowances</p>
          <p className="text-2xl font-bold text-green-600">${data.allowances?.toLocaleString() || 0}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-red-500">
          <p className="text-sm text-gray-500">Deductions</p>
          <p className="text-2xl font-bold text-red-600">${data.deductions?.toLocaleString() || 0}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-purple-500">
          <p className="text-sm text-gray-500">Tax Rate</p>
          <p className="text-2xl font-bold text-purple-600">{taxData.taxRate || 0}%</p>
          <p className="text-xs text-gray-500">Total Tax: ${taxData.totalTax?.toLocaleString() || 0}</p>
        </div>
      </div>
              ...
              data.records?.map((record: any) => (
                <tr key={record.id}>
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600">
                    {editingId === record.id ? (
                      <input 
                        ...
                        onChange={(e) => setEditData({...editData, basicSalary: parseFloat(e.target.value) || 0})} 
                      />
                  ...
                  <td className="px-6 py-4 text-sm font-medium text-green-600">
                    ${calculateNetPay(record.basicSalary || 0, record.allowances || 0, record.deductions || 0).toLocaleString()}
                  </td>
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-2">
          <button onClick={onGenerate} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">
            Generate Payroll
          </button>
          <button onClick={onRefresh} className="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600">
            🔄 Refresh
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-yellow-50 border-l-4 border-yellow-500 p-4 rounded">
          ...
          <button onClick={() => alert(`${stats.draft || 0} draft payrolls`)} className="text-xs text-yellow-600 hover:text-yellow-800 mt-1">
            View drafts →
          </button>
        </div>
        <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded">
          ...
          <button onClick={() => alert(`${stats.reviewed || 0} reviewed payrolls`)} className="text-xs text-blue-600 hover:text-blue-800 mt-1">
            View reviewed →
          </button>
        </div>
        <div className="bg-green-50 border-l-4 border-green-500 p-4 rounded">
          ...
          <button onClick={() => alert(`${stats.approved || 0} approved payrolls`)} className="text-xs text-green-600 hover:text-green-800 mt-1">
            View approved →
          </button>
        </div>
      </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {data?.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-6 py-8 text-center text-gray-500">
                  No payroll processing records found. Click "Generate Payroll" to start.
                </td>
              </tr>
            ) : (
              data?.map((record: any) => (
                <tr key={record.id}>
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600">{record.employees}</td>
                  <td className="px-6 py-4 text-sm text-gray-600">${record.total?.toLocaleString() || 0}</td>
                  ...
                  <td className="px-6 py-4 text-sm space-x-2">
                    {record.status === 'Draft' && (
                      <button onClick={() => onReview(record.id)} className="text-blue-600 hover:text-blue-800">Review</button>
                    )}
                    {record.status === 'Reviewed' && (
                      <button onClick={() => onApprove(record.id)} className="text-green-600 hover:text-green-800">Approve</button>
                    )}
            ...
            {data?.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-6 py-8 text-center text-gray-500">
                  No payslips found for {selectedMonth}. Click "Generate Payslip" to create one.
                </td>
              </tr>
            ) : (
              data?.map((payslip: any) => (
                <tr key={payslip.id}>
                  <td className="px-6 py-4 text-sm text-gray-900">{payslip.employee}</td>
                  <td className="px-6 py-4 text-sm text-gray-600">{payslip.month}</td>
                  <td className="px-6 py-4 text-sm font-medium text-green-600">${payslip.netPay?.toLocaleString() || 0}</td>
                  <td className="px-6 py-4 text-sm">
                    <span className={`px-2 py-1 rounded text-xs font-medium
        ...
        <nav className="-mb-px flex space-x-8">
          <button
            ...
            className={`py-2 px-1 border-b-2 font-medium text-sm
              ${historyTab === 'monthly' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500'}`}
          >
            Monthly Payroll Records
          </button>
          <button
            onClick={() => setHistoryTab('employee')}
            className={`py-2 px-1 border-b-2 font-medium text-sm
              ${historyTab === 'employee' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500'}`}
          >
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {data?.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-8 text-center text-gray-500">
                      No payroll history found for {selectedMonth}
                    </td>
                  </tr>
                ) : (
                  data?.map((record: any) => (
                    <tr key={record.id}>
                      ...
                      <td className="px-6 py-4 text-sm text-gray-600">{record.employees}</td>
                      <td className="px-6 py-4 text-sm font-medium text-gray-900">${record.total?.toLocaleString() || 0}</td>
                      <td className="px-6 py-4 text-sm">
                        <span className={`px-2 py-1 rounded text-xs font-medium
        ...
        {historyTab === 'employee' && (
          <div className="p-6">
            <div className="flex gap-2 mb-4">
              ...
              />
              <button onClick={onSearchEmployee} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">
                Search
              </button>
            </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {employeeHistory?.map((record: any) => (
                    <tr key={record.id}>
                      <td className="px-6 py-4 text-sm text-gray-900">{record.month}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">${record.basicSalary?.toLocaleString() || 0}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">${record.allowances?.toLocaleString() || 0}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">${record.deductions?.toLocaleString() || 0}</td>
                      <td className="px-6 py-4 text-sm font-medium text-green-600">${record.netPay?.toLocaleString() || 0}</td>
                      <td className="px-6 py-4 text-sm">
                        <span className={`px-2 py-1 rounded text-xs font-medium
