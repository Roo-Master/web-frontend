...
import { ArrowLeft, X, Check, Loader2, Percent } from 'lucide-react';
...
export default function AddTax() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await payrollService.createTax({
        ...formData,
        rate: parseFloat(formData.rate),
        bracketMin: formData.bracketMin ? parseFloat(formData.bracketMin) : undefined,
        bracketMax: formData.bracketMax ? parseFloat(formData.bracketMax) : undefined,
      });
      showToast('✅ Tax created successfully!', 'success');
      setTimeout(() => router.push('/payroll/taxes'), 1000);
    } catch (err: any) {
      showToast(err.message || 'Failed to create tax', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="max-w-2xl mx-auto">
        ...
        >
          <ArrowLeft className="w-4 h-4" /> Back to Taxes
        </button>

        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Add Tax</h1>
              <p className="text-gray-500 mt-1">Create a new tax configuration</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Bracket Min</label>
                <input 
                  type="number" 
                  step="0.01"
                  placeholder="e.g., 0"
                  value={formData.bracketMin}
                  onChange={(e) => setFormData({...formData, bracketMin: e.target.value})}
                  className="w-full bg-gray-50 text-gray-900 rounded-lg px-4 py-2 border border-gray-200 focus:outline-none focus:border-blue-500" 
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Bracket Max</label>
                <input 
                  type="number" 
                  step="0.01"
                  placeholder="e.g., 50000"
                  value={formData.bracketMax}
                  onChange={(e) => setFormData({...formData, bracketMax: e.target.value})}
                  className="w-full bg-gray-50 text-gray-900 rounded-lg px-4 py-2 border border-gray-200 focus:outline-none focus:border-blue-500" 
                />
              </div>
            </div>
            ...
            <div>
              ...
              <textarea 
                rows={3}
                placeholder="Provide a description of this tax..."
