...
export default function AddDeduction() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => router.push('/payroll/deductions'), 1000);
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="max-w-2xl mx-auto">
        ...
        >
          <ArrowLeft className="w-4 h-4" /> Back to Deductions
        </button>

        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Add Deduction</h1>
              <p className="text-gray-500 mt-1">Create a new employee deduction</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input 
                ...
                required
                step="0.01"
                placeholder={formData.type === 'percentage' ? 'e.g., 5' : 'e.g., 100'}
                value={formData.value}
                onChange={(e) => setFormData({...formData, value: e.target.value})}
                className="w-full bg-gray-50 text-gray-900 rounded-lg px-4 py-2 border border-gray-200 focus:outline-none focus:border-blue-500" 
              />
              <p className="text-xs text-gray-400 mt-1">
                {formData.type === 'percentage' ? 'Enter percentage (e.g., 5 for 5%)' : 'Enter amount in dollars'}
              </p>
            </div>
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input 
                    ...
                    onChange={() => setFormData({...formData, mandatory: true})}
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input 
                    ...
                    onChange={() => setFormData({...formData, mandatory: false})}
                    className="w-4 h-4 text-blue-600 focus:ring-blue-500"
                  />
            ...
            <div>
              ...
              <textarea 
                rows={3}
                placeholder="Provide a description of this deduction..."
